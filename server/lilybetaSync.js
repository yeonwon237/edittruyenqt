...
export function createLilyBetaSyncHandler({ env = process.env, fetchImpl = fetch } = {}) {
  return async function handler(req, res) {
    ...
    try {
      ...
      const { projectId, action, chapterIds } = req.body || {};
      ...
      const payload = JSON.stringify({ editorBookId: projectId, book: { title: project.title, totalChapters: metadata.length }, chapters });
